...
def load_model_from_checkpoint(checkpoint_path: str, se_model_path: str) -> tuple:
    ...
    with open(checkpoint_file, 'rb') as f:
        ...
        if header.startswith(b'<') or header.startswith(b'<!DOCTYPE'):
            raise ValueError(
                f"Checkpoint file appears to be HTML/text, not a valid PyTorch checkpoint: {checkpoint_path}\n"
...
def load_perturbation_features(features_path: str) -> dict:
    ...
    if not features_file.exists():
        raise FileNotFoundError(
            f"Perturbation features file not found: {features_path}\n"
            f"Please check the path is correct."
        )
    ...
    try:
        ...
        raise RuntimeError(
            f"Failed to load perturbation features from {features_path}\n"
            f"Error: {e}\n"
            f"The file may be corrupted or not a valid PyTorch tensor file."
        ) from e
